...
# paths
const PATH: String = "res://addons/scene_manager/scenes/scenes.gd"
...
const IGNORE_ITEM = preload("res://addons/scene_manager/src/ui/ignore_item/ignore_item.tscn")
const SCENE_LIST_ITEM = preload("res://addons/scene_manager/src/ui/scene_list/scene_list.tscn")
# icons
const HIDE_BUTTON_CHECKED = preload("res://addons/scene_manager/assets/icons/GuiChecked.svg")
const HIDE_BUTTON_UNCHECKED = preload(
	"res://addons/scene_manager/assets/icons/GuiCheckedDisabled.svg"
)
...
func get_all_sublists_names_except(_excepts: Array = [""]) -> Array:
...
func _get_scenes(root_path: String, ignores: Array) -> Dictionary:
	...
	if !(original_root_path in ignores) && dir:
		...
		# TODO: GODOT4 fill missing arguments https://github.com/godotengine/godot/pull/40547
		dir.list_dir_begin()
...
func _add_ignore_item(address: String) -> void:
	var item = IGNORE_ITEM.instantiate()
...
func _reload_scenes() -> void:
	...
	for key in data:
		...
		assert(
			...
			(
				"Scene Manager Error: this format is not supported. "
...
# Loads and returns just array value of `_ignore_list`
# key from `scenes` variable of `scenes.gd` file
...
func _on_hide_button_up():
	if _ignores_container.visible:
		_hide_button.icon = HIDE_BUTTON_UNCHECKED
		_ignores_container.visible = false
	else:
		_hide_button.icon = HIDE_BUTTON_CHECKED
