...
func _input(event):
	...
	if _check_current_screen_is_custom() and skip_custom_screen_type == SkipCustomNodeType.CUSTOM:
		return

	if skip_screen_type == SkipScreenType.NONE:
		return

	if skip_screen_type == SkipScreenType.SKIP_ONE_SCREEN_WHEN_CLICKED:
		status = TransitionStatus.NONE
		awe_timer.cancel()
		_on_finished_animation_screen_disappear()

	if skip_screen_type == SkipScreenType.SKIP_ALL_SCREEN_WHEN_CLICKED:
		status = TransitionStatus.NONE
...
func play_screen(screen):
	...
	if screen is AweSplashScreen:
		current_screen.finished.connect(self._on_finished_animation_splash_screen)
...
func _skip_awe_splash_by_event(_event) -> bool:
	return false
